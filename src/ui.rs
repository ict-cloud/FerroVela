        ...
        match save_config(&self.path, &config) {
            Ok(_) => {
                ...
                if self.upstream_use_keyring {
                    if !self.upstream_password.trim().is_empty() && !self.upstream_username.trim().is_empty() {
                        ...
                        if let Ok(entry) = entry {
                            if let Err(e) = entry.set_password(self.upstream_password.trim()) {
                                ...
                                log::info!("Successfully saved password to keyring for user '{}'", self.upstream_username.trim());
                            }
                        } else {
                    ...
                    if let Ok(entry) = keyring::Entry::new("ferrovela", self.upstream_username.trim()) {
